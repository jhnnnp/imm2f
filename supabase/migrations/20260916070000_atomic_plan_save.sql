-- Save a complete plan, its snapshot, and activity as one transaction.
-- revision_id provides optimistic concurrency for the two-person planner.
...
begin
  ...
  insert into public.plan_items (
    ...
    duration_minutes, expected_cost, sort_order, memo, day_index
  )
  select
    saved_plan.id,
    item.client_id,
...
revoke all on function public.save_couple_plan_atomic(text, text, text, date, integer, jsonb, text, bigint) from public;
grant execute on function public.save_couple_plan_atomic(text, text, text, date, integer, jsonb, text, bigint) to authenticated;
