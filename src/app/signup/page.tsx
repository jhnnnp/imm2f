...
import { isInviteToken, loginHrefForInvite } from "@/features/auth/invitePath";
...
export default async function SignupPage({ searchParams }: { searchParams: Promise<{ invite?: string }> }) {
  ...
  const inviteToken = isInviteToken(rawInvite) ? rawInvite : undefined;
  return (
    <AuthScreen
      title={inviteToken ? "초대를 받아 공간을 연결해요" : "우리의 공간을 만들어요"}
      description={inviteToken ? "가입하면 초대한 사람과 같은 공간에 들어가요." : "이메일로 계정을 만들고, 파트너를 초대해 주세요."}
      footer={<p>이미 있나요? <Link href={loginHrefForInvite(inviteToken)}>로그인</Link></p>}
    >
