...
import { PartnerManage } from "@/features/auth/components/PartnerManage";
import { getCoupleConnection } from "@/features/auth/session";
...
export default async function InvitePage() {
  const { session, connectedAt } = await getCoupleConnection();
  ...
  if (session.partner) {
    const connectedOn = connectedAt ? formatKoDate(connectedAt.slice(0, 10)) : null;
    return (
      <div className="invite-page">
        <div className="page-title-row invite-page-head">
          <div>
            <span className="eyebrow">TOGETHER</span>
            <h1>파트너</h1>
            <p>{pairLabel(session.displayName, session.partner.displayName)}의 공간입니다. 장소·일정·추억이 같은 화면에서 쌓여요.</p>
          </div>
        </div>
        <PartnerManage
